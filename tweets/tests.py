...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        Tweet.objects.create(content="my first-1 tweet", user=self.user)
        Tweet.objects.create(content="my first-2 tweet", user=self.userb)
        self.currentCount = Tweet.objects.all().count()

    def test_tweet_created(self):
        tweet_obj = Tweet.objects.create(content='MY second Tweet test', user=self.user)
    ...
    def test_tweets_releated_name(self):
        user = self.user
    ...
    def test_action_like(self):
        ...
        my_releated_likes = user.tweet_user.count()
        self.assertEqual(my_like_instances_count, my_releated_likes)
        # print(response.json())
        # self.assertEqual(len(response.json()), 3)
